...
```bash
python -m unittest discover -s tests   # 93개 — 외부 AI 교차 게이트의 회귀 고정
```

## 알려진 한계 (정직하게)

- **회전 콘텐츠 사이트의 클로킹 확정은 최대 4패스 지연된다.** 요청마다 배너·문구가 바뀌는 페이지에서는 "유한 표본에서 안 보였다"가 결론이 될 수 없어, 여러 패스에 걸쳐 재현될 때만 🔴으로 확정한다. 그 전에도 침묵하지 않고 🟠 "클로킹 의심 (n/4패스 확인 중)"으로 알린다. 회전이 없는 안정 페이지는 지연 없이 그 패스에서 확정한다
- **클로킹 판정의 위상 보장은 확률적이다** — "절대 안 뚫린다"가 아니라 "패스가 쌓일수록 오탐 확률이 0에 수렴한다". 실측(주기 2~16, 시작 위상 전수 × 6패스 = 882패스): 수리 전 하드 FAIL 74건 → 수리 후 0건. 요청마다 낮은 확률로 도박 문구가 끼는 페이지에서는 수리 전 3.3%/패스 → 수리 후 0~0.12%/패스 (완전 0은 아니다)
