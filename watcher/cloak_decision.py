...
## 탐지 하한 — 실측으로 확정한 경계 (숨기지 않는다)

확정은 순증 누적이라 **패스당 후보를 볼 확률이 1/2를 넘어야** 위로 드리프트한다.
한 패스에 봇 표본을 B회(5~12) 뜨므로, 노출이 **N회에 1번 결정적으로** 오는
클로커는 그 패스에 볼 확률이 `min(1, B/N)`이고 조건은 대략 `N < 2·E[B] ≈ 17`이다.
⚠️여기에 `1-(1-r)^B`(독립 시행 법칙)를 쓰면 안 된다 — 20차에 그렇게 써서 하한을
1/12로 잘못 적었다. 결정적 노출과 독립 노출은 다른 법칙이고, 실측은 결정적 쪽이다.

**실측 경계 (노출률 × 봇 시작 위상 전수 × 24패스)**
...
즉 실효 하한은 **1/13~1/14**다. 그 아래에서는 🔴까지 가지 않고, 게다가 봇 표본이
그 회차에 스팸을 못 보면 후보 자체가 안 생겨 **🟢으로 보고될 수 있다**(1/20에서
전 위상 평균 3.8%, 특정 시작 위상에서는 대부분의 패스). "의심에 머문다"가 아니라
"못 보면 초록"이다 — 20차 P2-1에서 반증된 서술이니 다시 세게 쓰지 않는다.

하한을 낮추려면 봇 표본을 늘려야 하는데 그건 감시 대상 사이트의 부담과 직결된다.
검색 노출이 목적인 클로커가 봇에게 1/15만 보여주는 건 스스로 목적을 깎는 짓이라
여기서 균형을 잡았다.
...
# 확정에 필요한 **순증 누적 점수**(경과 패스 상한이 아니다 — 관측 없는 패스는
# 감쇠하므로 실제 경과 시간은 무계다). 회전 여부로 나누지 않는다 — 19차 P2-1:
# 임계를 그 패스의 회전 관측으로 정하면 누적은 공유되는데 잣대만 흔들려, 아무것도
# 관측되지 않은 패스에서 감쇠(5→4)가 낮아진 임계를 넘어 확정되거나, 후보를 다시
# 본 패스가 오히려 🔴→🟠 강등이 된다. 하나로 통일하는 쪽이 그 클래스를 없앤다.
    ...
    for kind in _KINDS:
        ...
        for key, streak in bucket.items():
            if (isinstance(key, str) and isinstance(streak, int)
                ...
                # 상한으로 클램프 — 옛 버전이 남긴(또는 손상된) 큰 값이 그대로 들어오면
                # 해제까지 필요한 깨끗한 패스 수가 설계보다 길어진다 (20차 P3-3)
                memory[kind][key] = min(streak, STREAK_CAP)
    return memory
...
def _age(bucket, alive, disproved):
    ...
    누적 상한 = STREAK_CAP(= CONFIRM_PASSES + 2) → 확정 해제도 몇 패스의 깨끗한
    관측을 요구한다(한 번 깨끗했다고 곧장 초록으로 돌아가는 비대칭 복구 금지).
    """
