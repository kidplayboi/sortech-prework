...
- **축 2 패스 간 누적** — 한 패스로 확정하지 않는다. CONFIRM_PASSES회 **연속**
  재현될 때만 확정하고 그 전에는 WARN(의심 n/K)로 노출한다. 연속을 요구하는
  이유는 결정적 회전이 비연속 누적을 그대로 채우기 때문이다(비연속 허용 시
  주기 12에서 36% 확률로 확정됨을 산식으로 확인).
...
def evaluate(spam_candidates, marker_candidates, user_texts, bot_texts, memory,
    ...
    for kind in _KINDS:
        ...
        for key in alive[kind]:
            bucket[key] = min(bucket[key] + 1, confirm_passes * 2)
            if _immediate(kind, key, rotating, bot_hits, len(bot_texts)):
                bucket[key] = max(bucket[key], confirm_passes)
        verdict[kind + "_confirmed"] = [k for k, n in bucket.items() if n >= confirm_passes]
        verdict[kind + "_suspect"] = [(k, n) for k, n in bucket.items() if n < confirm_passes]
...
def _age(bucket, alive, disproved):
    ...
    - **미관측**(이번 봇 표본엔 안 나왔다) = 증거의 부재일 뿐 → 1씩 감쇠.
      ...
      감쇠라서 결정적 회전은 여전히 누적을 못 쌓는다(escalation 사이에 조용한
      패스가 끼므로 순증이 0 이하).
